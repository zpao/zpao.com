...
Progress on this was not as fast as I’d hoped it would be. It got stalled behind some other work, going to the CMU job fair, [I was poisoned](https://twitter.com/zpao/status/5076044119), and I had to figure out how the focus tests work. But that’s enough with the negativity, this is supposed to be good news.

I finally got all my ducks in row and [landed the feature today](https://hg.mozilla.org/mozilla-central/rev/bbeaa4e518ee)! If you don’t remember what the feature does (or never knew), read through [my original article on it](/posts/per-tab-network-prioritization/). While the underlying implementation changed a fair amount, the basic concept is still the same. The only change made is that we adjust the priority instead of just changing it to pre-determined values. By doing it this way, we won’t interfere with an add-on that, for example, gave Gmail a super high network priority.
